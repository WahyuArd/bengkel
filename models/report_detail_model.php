...
function select_detail_purchases($id,$branch_id){
	$query = mysql_query("SELECT a.*, b.*, c.*, a.purchase_total AS p_tot, c.item_name
		 										FROM purchases a
												LEFT JOIN purchase_details b ON b.purchase_id = a.purchase_id
												LEFT JOIN items c ON c.item_id = a.item_id
												WHERE a.purchase_code = '$id' AND a.branch_id = '$branch_id'");
  return $query;
}
...
function update_stock($id ,$qty, $branch_id){
  ...
  function select_item_purchase($id){
    $query = mysql_query("SELECT a.*, b.*, a.purchase_price AS harga_item, a.purchase_total AS harga_item_total,  c.* FROM purchases a
                          LEFT JOIN purchase_details b ON b.purchase_id  = a.purchase_id
