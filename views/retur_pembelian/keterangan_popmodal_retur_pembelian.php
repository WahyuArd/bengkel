  ...
  <div class="modal-body">
      <table id="tb_kategori_keterangan" class="table-bordered table-striped" style="width:100%;">
        ...
        <tbody>
          ...
          <input type="" id="purchase_id" name="purchase_id" value="<?= $purchase_id?>" style="display:none;">
          <input type="" id="wr_pembelian_tmp_id" name="wr_pembelian_tmp_id" value="<?= $wr_pembelian_tmp_id?>" style="display:none;">
...
$(document).ready(function() {
  ...
  function select_picker(id) {
    ...
    var kategori_keterangan_id = $('#kategori_keterangan_id').val();
      ...
      $.ajax({
        type:'POST',
        data:{item_id:item_id,kategori_keterangan_id:kategori_keterangan_id, purchase_id:purchase_id},
        url: 'retur_pembelian.php?page=kategori_keterangan',
        dataType:'json',
      }).done(function(data){
        $('#selectpicker_'+id).empty();
        $('#selectpicker_'+id).append('<option value="0"></option>');
        for (var i = 0; i < data.length; i++) {
          $('#selectpicker_'+id).append('<option value="'+data[i].item_keterangan_details_id+'">'+data[i].keterangan_details+'</option>');
        }
